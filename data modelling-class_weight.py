...
def feature_ranking(model, X_train_resampled, y_train_resampled, feature_names, model_name, fold_number, thr):
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        plt.title(f'Feature Importances for {model_name} (Fold {fold_number})', fontsize=4.5)
        plt.xlabel('Importance', fontsize=4.5)
        plt.ylabel('Features', fontsize=4.5)
        ...
        plt.title(f'Cumulative Importance of Features for {model_name} (Fold {fold_number})', fontsize=4.5)
        plt.xlabel('Number of Features', fontsize=4.5)
        plt.ylabel('Cumulative Importance', fontsize=4.5)
        plt.xticks(fontsize=3)
        plt.yticks(fontsize=3)
        plt.show()
...
def cross_validate_model(model, model_name, X, y, thr=0.99):
    ...
    for fold_number, (train_index, test_index) in enumerate(skf.split(X, y), 1):
        ...
        selected_features = feature_ranking(lgb_model, X_train_scaled, y_train, feature_names, model_name, fold_number, thr)
        ...
        best_model = hyperparameter_tuning(model, param_grids[model_name], X_train_selected, y_train)
...
# We train and evaluate several models including Random Forest, XGBoost, LightGBM, and Logistic Regression using the cross-validation function defined earlier. To address the imbalanced classes, Random Forest, LightGBM, and Logistic Regression are initialized with a balanced class weight. In XGBoost, balanced class weights are typically handled through the scale_pos_weight parameter.
...
xgb_model = xgb.XGBClassifier(random_state=42, use_label_encoder=False, eval_metric='logloss', scale_pos_weight=class_counts[0] / class_counts[1])
